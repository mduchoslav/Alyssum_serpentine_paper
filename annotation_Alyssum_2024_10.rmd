...
# Renaming old version and making new folders
I will leave below the commands how to get some of the files that were here just copied from previous version.
```{sh}
# rename old version
cd /storage/brno12-cerit/home/duchmil/annotations/
mv alyssum_2024_Mahnaz_assembly/ old_version_01_alyssum_2024_Mahnaz_assembly
# make new folder and subfolders
mkdir alyssum_2024_Mahnaz_assembly
mkdir alyssum_2024_Mahnaz_assembly/genome_assembly
mkdir alyssum_2024_Mahnaz_assembly/rnaseq
cp -Rv old_version_01_alyssum_2024_Mahnaz_assembly/rnaseq/1_raw_reads/ old_version_01_alyssum_2024_Mahnaz_assembly/rnaseq/2_trimmed_reads/ alyssum_2024_Mahnaz_assembly/rnaseq

cp -Rv old_version_01_alyssum_2024_Mahnaz_assembly/braker_sw alyssum_2024_Mahnaz_assembly/
cp -Rv old_version_01_alyssum_2024_Mahnaz_assembly/protein_seqs_input alyssum_2024_Mahnaz_assembly/

mkdir alyssum_2024_Mahnaz_assembly/metacentrum_scripts
cp -v old_version_01_alyssum_2024_Mahnaz_assembly/metacentrum_scripts/*FastQC* old_version_01_alyssum_2024_Mahnaz_assembly/metacentrum_scripts/*fastqc* old_version_01_alyssum_2024_Mahnaz_assembly/metacentrum_scripts/trim* alyssum_2024_Mahnaz_assembly/metacentrum_scripts
...
~/SW/stringtie/stringtie -p 16 -o Alyssum_assembled_transcripts.gtf Alyssum_RNAseq_trimmed_merged.bam
```
...
The proteins and CDSs should be same as for alyssum_01_changed_IDs.gff and alyssum_annotation_01.gff, just the IDs changed, so I will extract it again.
...
# Checking Orthofinder results

```{r}
setwd("D:/!ecolgen/resources/orthofinder/brassicaceae_1/")

# Sonia's candidate genes
candidates1 <- read.table(file = "R_analysis/Sonia_Alyssum_candidates/FstCandidates_AM019AM018.txt", header = T)
summary(candidates1)

# Orthologues table
ortho <- read.table(file = "orthofinder_results/Results_brassicaceae_1/Orthologues/Orthologues_Alyssum_gmelinii/Alyssum_gmelinii__v__Arabidopsis_thaliana.tsv", header = T, sep = "\t")
summary(ortho)

# Assignment of orthologues from ortho table
for(i in 1:nrow(candidates1)) {
  gene.row <- grep(pattern = candidates1[i, 1], x = ortho$Alyssum_gmelinii)
  if(length(gene.row) > 1) stop("Too many matches for gene: ", candidates1[i, 1])
  if(length(gene.row) == 1) candidates1[i, 5:7] <- ortho[gene.row, ]
}

## Extraction of information, if the gene is annotated as protein-coding or non-coding
# Genome annotation
gff.5 <- read.table(file = "D:/!ecolgen/annotations/Alyssum/Alyssum_assembly_from_Mahnaz_2024_05/annot_processing/alyssum_v2_annotation_old_1.gff", header = F, sep = "\t", comment.char = "#" 
                     #, nrows = 1500
                    )
# Vector of gene IDs
gff.genes <- gsub(pattern = "ID=|;gene_id.*$", replacement = "", x = gff.5[grepl(pattern = "gene", x = gff.5$V3), "V9"])
# Vector of GFF features ("gene" or "ncRNA_gene")
gff.type <- gff.5[grepl(pattern = "gene", x = gff.5$V3), "V3"]

# Assignment of gene types to candidates
for(i in 1:nrow(candidates1)) {
  gene.row <- grep(pattern = candidates1[i, 1], x = gff.genes)
  if(length(gene.row) > 1) stop("Too many matches for gene: ", candidates1[i, 1])
  if(length(gene.row) == 1) candidates1$gene_type[i] <- gff.type[gene.row]
}

# remove last missing one
candidates1 <- candidates1[candidates1$Var1 != "AgNA", ]

## Statistics
# Number of candidates
dim(candidates1)

# Number of candidates with orthologues assigned
sum(!is.na(candidates1$Arabidopsis_thaliana.1))

# Numbers of gene types
table(candidates1$gene_type)

# Numner of protein coding genes without assigned orthologue
sum(is.na(candidates1$Arabidopsis_thaliana.1) & candidates1$gene_type == "gene")
```


# Testing different numbers of RNA samples
Steps
- merge different sets of aligned RNAseq samples (samtools)
- assemble transcripts (Stringtie)
- run bedtools intersect to compare assembled transcripts (with whole set)
- extract transcript sequences (AGAT)
- run BUSCO
- run BRAKER with some of the variants?
...
- zkontrolovat pomocí AGAT
- exportovat proteiny a CDS
...
agat_sp_filter_feature_from_keep_list.pl --gff ../rnaseq/4_assembled_transcripts/Alyssum_assembled_transcripts.gtf --keep_list Alyssum_assembled_transcripts_with_no_annotation_overlap.txt -o Alyssum_assembled_transcripts_with_no_annotation_overlap_clean.gff

agat_sp_filter_feature_from_keep_list.pl --type transcript --gff ../rnaseq/4_assembled_transcripts/Alyssum_assembled_transcripts.gtf --keep_list Alyssum_assembled_transcripts_with_no_annotation_overlap.txt -o Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_2.gff

grep -P -c "\ttranscript\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean.gff # 31088
grep -P -c "\ttranscript\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_2.gff # 31088
# I don't know why is there more transcripts than in keep list.

grep -P "\ttranscript\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean.gff | sed -E 's/^.*;transcript_id=//g' | head
grep -P "\ttranscript\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean.gff | sed -E 's/^.*;transcript_id=//g' | sort | head -n 100

cat Alyssum_assembled_transcripts_with_no_annotation_overlap.txt  | sort | head  -n 100

paste <(cat Alyssum_assembled_transcripts_with_no_annotation_overlap.txt  | sort | head  -n 100) <(grep -P "\ttranscript\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean.gff | sed -E 's/^.*;transcript_id=//g' | sort | head -n 100)


agat_convert_sp_gxf2gxf.pl -g Alyssum_assembled_transcripts_with_no_annotation_overlap.gtf -o Alyssum_assembled_transcripts_with_no_annotation_overlap.gff

agat_sp_filter_feature_from_keep_list.pl --type transcript --gff Alyssum_assembled_transcripts_with_no_annotation_overlap.gff --keep_list Alyssum_assembled_transcripts_with_no_annotation_overlap.txt -o Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_3.gff
agat_sp_filter_feature_from_keep_list.pl --type transcript --gff Alyssum_assembled_transcripts_with_no_annotation_overlap.gtf --keep_list Alyssum_assembled_transcripts_with_no_annotation_overlap.txt -o Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_4.gff
agat_sp_filter_feature_from_keep_list.pl --gff Alyssum_assembled_transcripts_with_no_annotation_overlap.gtf --keep_list Alyssum_assembled_transcripts_with_no_annotation_overlap.txt -o Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_5.gff
agat_sp_filter_feature_from_keep_list.pl --type transcript,RNA --gff Alyssum_assembled_transcripts_with_no_annotation_overlap.gtf --keep_list Alyssum_assembled_transcripts_with_no_annotation_overlap.txt -o Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_6.gff

grep -P -c "\ttranscript\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_3.gff
grep -P -c "\ttranscript\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_4.gff

grep -P -c "\tRNA\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_4.gff

grep -P -c "\ttranscript\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_5.gff
grep -P -c "\tRNA\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_5.gff

grep -P -c "\ttranscript\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_6.gff
grep -P -c "\tRNA\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_6.gff
grep -P "\tRNA\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_6.gff | head -n 100
grep -P "\tRNA\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_6.gff | sed -E 's/^.*;transcript_id=//g' | head -n 100

agat_sp_filter_feature_from_kill_list.pl --gff Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_6.gff --kill_list <(grep -P "\tRNA\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_6.gff | sed -E 's/^.*;transcript_id=//g') -o Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_7.gff


grep -P -c "\ttranscript\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_7.gff
grep -P -c "\tRNA\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_7.gff

# Still not solved!!!

grep -P "\tRNA\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_6.gff | sed -E 's/^.*;transcript_id=//g' > transcript_kill_list.txt

agat_sp_filter_feature_from_kill_list.pl --gff Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_6.gff --kill_list transcript_kill_list.txt -o Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_8.gff

grep -P -c "\ttranscript\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_8.gff
grep -P -c "\tRNA\t" Alyssum_assembled_transcripts_with_no_annotation_overlap_clean_8.gff
